                   ...
                   <div className='h-full  flex items-start justify-center mt-3 mr-3'>
                            <form className='py-6 ml-3 w-full'>
                                ...
                                <div className='relative'>
                                    ...
                                    {email ? "" : <AiOutlineUser className='absolute bottom-8 right-3 text-xl text-gray-400' />}
                                </div>
                                <div className='relative'>
                                ...
                                <div className='relative'>
                                    ...
                                    {showPassword ? <AiOutlineEyeInvisible className='absolute bottom-3 right-3 text-xl text-gray-400 cursor-pointer' onClick={() => { setShowPassword(false) }} /> : <AiOutlineEye className='absolute bottom-8 right-3 text-xl text-gray-400 cursor-pointer' onClick={() => { setShowPassword(true) }} />}
                                </div>

                                <div className='relative'>
                                    <input type={showPassword ? "text" : "password"} className='w-full font-normal border rounded-md  border-gray-200  p-2 mb-2' placeholder='Re-enter password' id='confirmPassword' value={confirmPassword} onChange={handleInput} />
                                    {showPassword ? <AiOutlineEyeInvisible className='absolute bottom-3 right-3 text-xl text-gray-400 cursor-pointer' onClick={() => { setShowPassword(false) }} /> : <AiOutlineEye className='absolute bottom-5 right-3 text-xl text-gray-400 cursor-pointer' onClick={() => { setShowPassword(true) }} />}
                                </div>
